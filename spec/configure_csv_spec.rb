...
describe 'Configure CSV' do
  context 'get header from csv' do
    it { expect(ConfigureCSV.set_header).to include 'cpf' }
    it { expect(ConfigureCSV.set_header).to include 'nome paciente' }
    it { expect(ConfigureCSV.set_header).to include 'crm médico' }
    it { expect(ConfigureCSV.set_header).to include 'nome médico' }
  end
  ...
  it 'array data into hash' do
    ...
    Database.connect.exec_params("insert into patient(cpf, name, email, birthdate, address, city, state, crm, crm_state,
                                ...
                                exam_result) values ('048.973.170-88','Emilly Batista Neto',
                                'gerald.crona@ebert-quigley.com','2001-03-11','165 Rua Rafaela', 'Ituverava','Alagoas',
                                'B000BJ20J4','PI','Maria Luiza Pires','denna@wisozk.biz','IQCZ17','2021-08-05',
                                'hemácias','45-52','97')")
    SetDatabase.select_table
